...
def wx_login():
    data = request.json
    openid = data.get('code', '') or 'test_openid_' + hashlib.md5(str(datetime.now()).encode()).hexdigest()[:8]
    db = get_db()
    try:
...
def recommended_tasks():
    ...
    lat = float(request.args.get('lat', 40.5))
    lng = float(request.args.get('lng', 122.2))
    db = get_db()
    try:
        with db.cursor() as cur:
            ...
            if user and user.get('fitness_tags'):
                ...
                cur.execute(f"""
                    ...
                    FROM tasks t
                    LEFT JOIN task_rewards tr ON t.id = tr.task_id
            ...
            for t in tasks:
                t['estimated_calories'] = calc_calories(
                    t['fitness_tag'],
                    t.get('distance_meters', 0),
                    t.get('estimated_min', 15),
                    weight_kg=weight
                )
                t['exercise_benefit'] = EFFECT_MAP.get(t['fitness_tag'], '全身锻炼')
                t['steps_equivalent'] = STEPS_MAP.get(t['fitness_tag'], lambda x: 0)(t.get('distance_meters', 0) or t.get('estimated_min', 0) * 200)
...
def task_detail(task_id):
    ...
    try:
        with db.cursor() as cur:
            cur.execute("""
                SELECT t.*, tr.commission_amount, tr.coupon_desc, tr.platform_points,
                       m.company_name, m.address as merchant_address
                FROM tasks t
                LEFT JOIN task_rewards tr ON t.id = tr.task_id
            ...
            task['estimated_calories'] = calc_calories(task['fitness_tag'], task.get('distance_meters', 0), task.get('estimated_min', 15))
...
def nearby_tasks():
    lat = request.args.get('lat', 40.5, type=float)
    lng = request.args.get('lng', 122.2, type=float)
...
def calc_calories_api():
    ...
    return jsonify({
        ...
        "data": {
            ...
            "formula": f"{tag}类型 · 体重{weight}kg · {'距离'+str(distance)+'m' if distance else ''}{'时长'+str(minutes)+'min' if minutes else ''}"
        }
    })
