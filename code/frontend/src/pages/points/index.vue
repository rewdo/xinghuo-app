<template>
	<view class="page">
		...
		<!-- Tab 切换 -->
		<view class="tab-bar">
			<view class="tab-item" :class="{active: activeTab === 'shop'}" @tap="activeTab = 'shop'">
				<text>🏪 积分商城</text>
			</view>
			<view class="tab-item" :class="{active: activeTab === 'history'}" @tap="activeTab = 'history'">
				<text>📋 积分流水</text>
			</view>
		</view>
		...
		<view class="shop-list" v-if="activeTab === 'shop'">
			<view class="shop-card" v-for="item in shopItems" :key="item.id" @tap="redeem(item)">
				<view class="shop-emoji">{{ item.emoji || '🎁' }}</view>
		...
		async loadData() {
			try {
				const res = await uni.request({
					url: 'http://localhost:5000/api/v1/points/balance?user_id=1',
					timeout: 3000
				})
				if (res.data && res.data.code === 0) {
					this.points = { ...this.points, ...res.data.data }
				}
				const res2 = await uni.request({
					url: 'http://localhost:5000/api/v1/points/shop/items',
					timeout: 3000
				})
				if (res2.data && res2.data.code === 0 && res2.data.data.length > 0) {
					this.shopItems = res2.data.data
				}
			} catch (e) {}
		},
		redeem(item) {
